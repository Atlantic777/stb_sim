...
int sdt_parse(sdt_table_t *sdt, uint8_t *buff)
{
	...
	sdt->original_network = get_short(buff, 8);
	sdt->reserved_futures_use = buff[10];
	...
	while(cursor*8 < sdt->section_length)
	{
		...
		current_item->descriptors = malloc(sizeof(service_descriptor_t));
		
		current_descriptor = current_item->descriptors;
